...
class HeatLadleConsumptionTable(Document):
	def validate(self):
		"""Override validation if parent Heat document has downtime checked"""
		frappe.log_error(f"Heat-Ladle Consumption Table validate called. Parent: {self.parent}, ParentType: {self.parenttype}", "Heat Debug")
		...
		if self.parent and self.parenttype == "Heat":
			try:
				...
				if downtime_status:
					frappe.log_error(f"Skipping Heat-Ladle Consumption Table validation due to downtime mode in Heat {self.parent}", "Heat Debug")
					return
		...
		if downtime_mode:
			frappe.log_error(f"Skipping Heat-Ladle Consumption Table validation due to global downtime mode flag", "Heat Debug")
			return
		
		# Normal validation will proceed if not in downtime mode
		frappe.log_error("Proceeding with normal Heat-Ladle Consumption Table validation", "Heat Debug")
