...
frappe.pages['heat-dashboard'].on_page_load = function (wrapper) {
    ...
    const $filterBar = $('<div class="heat-filters" style="display:flex;gap:12px;align-items:end;flex-wrap:wrap;margin-bottom:12px;"></div>');
    ...
    $filterBar.append($fromWrap).append($toWrap).append($furnaceWrap).append($btnWrap);
    ...
    $(state.controls.furnace_no.$input).on('change', () => refresh(state));
...
function render_cards(state, summary) {
    ...
    summary.forEach((card) => {
        ...
        const value = (card.datatype === 'Float') ? format_number(card.value || 0, null, 3) : format_number(card.value || 0);
        const $card = $(`
