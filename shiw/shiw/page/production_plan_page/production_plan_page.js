...
if (!frappe.pages['production-plan-page']) {
    frappe.pages['production-plan-page'] = {};
}

frappe.pages['production-plan-page'].on_page_load = function (wrapper) {
    console.log('Production Plan Dashboard page loading...');

    // Build page shell
    const page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'Production Plan Dashboard',
        single_column: true,
    });
...
function refreshDashboard(state) {
    console.log('Refreshing production plan dashboard...');
...
function exportTableData() {
    ...
    link.setAttribute('download', `production_plan_${new Date().toISOString().split('T')[0]}.csv`);
