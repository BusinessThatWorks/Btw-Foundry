...
class Chargemixcomponenttable(Document):
	def validate(self):
		...
		frappe.log_error(f"Charge mix component table validate called. Parent: {self.parent}, ParentType: {self.parenttype}", "Heat Debug")
		...
		if self.parent and self.parenttype == "Heat":
			try:
				...
				if downtime_status:
					frappe.log_error(f"Skipping Charge mix component table validation due to downtime mode in Heat {self.parent}", "Heat Debug")
					return
		...
		if downtime_mode:
			frappe.log_error(f"Skipping Charge mix component table validation due to global downtime mode flag", "Heat Debug")
			return
