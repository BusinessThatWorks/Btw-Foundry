...
if (!frappe.pages['mould-dashboard']) {
    frappe.pages['mould-dashboard'] = {};
}

frappe.pages['mould-dashboard'].on_page_load = function (wrapper) {
    console.log('Mould Dashboard page loading...');

    // Build page shell like ERPNext's sales_funnel
    const page = frappe.ui.make_app_page({
        parent: wrapper,
        title: __('Mould Dashboard'),
        single_column: true,
    });
    ...
    const $filterBar = $('<div class="mould-filters" style="display:flex;gap:12px;align-items:end;flex-wrap:wrap;margin-bottom:12px;justify-content:space-between;"></div>');
    ...
    const $doctypeWrap = $('<div style="min-width:220px;"></div>');
    const $btnWrap = $('<div style="display:flex;align-items:end;"></div>');
    $filterControls.append($fromWrap).append($toWrap).append($doctypeWrap);
    ...
    state.controls.doctype_name = frappe.ui.form.make_control({
        parent: $doctypeWrap.get(0),
        df: {
            fieldtype: 'Select',
            label: __('Batch Type'),
    ...
    state.controls.doctype_name.set_value('All');
    ...
    $(state.controls.doctype_name.$input).on('change', () => refresh(state));
...
frappe.pages['mould-dashboard'].on_page_show = function () {
    console.log('Mould Dashboard shown');
};
...
function refresh(state) {
    ...
    const doctype_name = state.controls.doctype_name.get_value();

    console.log('Filter values:', { from_date, to_date, doctype_name });
    ...
    if (doctype_name && doctype_name !== 'All') {
...
function render_cards(state, summary) {
    ...
    summary.forEach((card) => {
        ...
        const value = (card.datatype === 'Float') ? format_number(card.value || 0, null, card.precision || 3) : format_number(card.value || 0);
        const $card = $(`
...
function indicator_color(indicator) {
    switch (indicator) {
        ...
        case 'orange': return 'linear-gradient(90deg,#f39c12,#e67e22)';
        case 'blue':
